...
export default function AIProgressInsight({ insight }) {
  ...
  if (!insight) return (
    <div className="rounded-2xl p-5 h-full flex flex-col" style={{ background: '#F5F3FF', border: '1px solid #EDE9FE', boxShadow: 'var(--th-shadow)' }}>
      <div className="flex items-center gap-2 mb-4">
        <div className="w-6 h-6 rounded flex items-center justify-center bg-violet-500/10">
          <Sparkles className="w-3.5 h-3.5 text-violet-600" />
        </div>
        <h3 className="text-sm font-bold text-violet-900">AI Progress Insight</h3>
      </div>
      <p className="text-xs text-violet-800 mb-4 font-medium leading-relaxed">Log body metrics and workouts to get AI-powered progress analysis.</p>
    </div>
  );
  ...
  return (
    <>
      <div className="rounded-2xl p-5 h-full flex flex-col" style={{ background: '#F5F3FF', border: '1px solid #EDE9FE' }}>
        ...
        {d.summary && <p className="text-xs text-violet-800 mb-4 font-medium leading-relaxed line-clamp-3">{d.summary}</p>}
            ...
            >
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-violet-950">Detailed Analysis</h2>
                    <p className="text-xs font-semibold text-violet-700">AI Progress Insight</p>
                  </div>
                </div>
