    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} layout>
      <Card className="group">
        <div className="flex items-start justify-between gap-3 mb-2">
          ...
          <button onClick={() => onDelete?.(workout.id)} className="opacity-0 group-hover:opacity-100 p-1 rounded text-zinc-600 hover:text-danger transition-all">
          ...
          <div className="pt-2 border-t border-white/[0.04] space-y-1">
            {workout.exercises.slice(0, 4).map((ex, i) => (
              <div key={i} className="flex items-center justify-between text-xs">
                ...
                <span className="text-zinc-500 stat-number">
            ...
            {workout.exercises.length > 4 && <p className="text-[10px] text-zinc-600">+{workout.exercises.length - 4} more</p>}
          </div>
        )}

        {workout.notes && <p className="text-[10px] text-zinc-600 mt-2 italic">{workout.notes}</p>}
      </Card>
    </motion.div>
  );
}
